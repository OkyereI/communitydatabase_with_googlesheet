...
#     Handles authentication using a service account.
#     """
...
#     service_account_path = app.config.get('GOOGLE_SERVICE_ACCOUNT_PATH')
#     google_sheet_url = app.config.get('GOOGLE_SHEET_URL')

#     if not service_account_path or not os.path.exists(service_account_path):
#         app.logger.error(f"Google service account key path not found or invalid: {service_account_path}")
#         flash("Google Sheets integration disabled: Service account key not found.", "error")
#         return None, None
    
#     if not google_sheet_url:
#         app.logger.error("Google Sheet URL not configured.")
#         flash("Google Sheets integration disabled: Sheet URL not configured.", "error")
#         return None, None

#     try:
#         creds = Credentials.from_service_account_file(service_account_path, scopes=SCOPES)
#         _gspread_client = gspread.authorize(creds)
#         _google_sheet = _gspread_client.open_by_url(google_sheet_url).sheet1 # Assumes working with the first sheet
#         app.logger.info("Successfully connected to Google Sheet.")
#         return _gspread_client, _google_sheet
#     except Exception as e:
#         app.logger.error(f"Failed to initialize Google Sheets client: {e}")
#         flash(f"Error connecting to Google Sheet: {e}", "error")
#         return None, None
...
#         self.offset = self.page * per_page
...
#                         # If sheet is empty or headers mismatch, try to update/add header row
#                         # This is a robust check, but be cautious with existing data.
#                         # For a new sheet, it will add headers. For an existing sheet with different headers,
#                         # it might not align perfectly unless you clear the sheet first.
#                         if not current_sheet_headers: # Only add if sheet is genuinely empty
#                             gs_sheet.append_row(headers)
#                             app.logger.info("Google Sheet header row added.")
#                         else:
#                             flash("Google Sheet headers mismatch. Data might be misaligned in the sheet.", "warning")
...
#                         model.residence,
#                         model.area_code,
#                         'Yes' if model.is_verified else 'No', # Convert boolean to string
...
#                     flash(f"Error appending to Google Sheet: {gs_ex}", "error")
...
#             flash("Google Sheet for updates not implemented yet. Only new records are appended.", "info")
...
#             'Employer': member.employer, # Added
#             'Parent/Guardian Name': member.parent_guardian_name, # Added
#             'Parent/Guardian Contact': member.parent_guardian_contact, # Added
#             'Parent/Guardian Address': member.parent_guardian_address, # Added
#             'Date of Birth': member.date_of_birth.strftime('%Y-%m-%d') if member.date_of_birth else 'N/A',
#             'Residence': member.residence,
#             'Area Code': member.area_code,
#             'Is Verified': 'Yes' if member.is_verified else 'No', # Added
...
class LoginForm(FlaskForm):
    username = StringField('Username', validators=[DataRequired()])
    password = PasswordField('Password', validators=[DataRequired()])
    remember_me = BooleanField('Remember Me') # Correctly defined here
    submit = SubmitField('Login')
...
class CommunityMemberView(ModelView):
    ...
    def _actions_formatter(self, context, model, name):
        ...
        delete_url = self.get_url('.delete_view', id=model.id, url=self.get_save_return_url(model, False))
        send_sms_url = self.get_url('.send_sms_view', member_id=model.id)
        # Corrected URL for print_member_info route
        print_url = url_for('print_member_info', member_id=model.id) 
    ...
    def send_sms_view(self, member_id):
        ...
        class IndividualSMSForm(FlaskForm): # This should probably be outside the method too, but for now...
    ...
    def send_all_messages_view(self):
        ...
        if form.validate_on_submit():
            ...
            return redirect(url_for('admin.index')) # Corrected: use admin.index directly if this is for the main dashboard
...
admin = Admin(app, name='Community Portal', template_mode='bootstrap3', index_view=MyAdminIndexView())
admin.add_view(ModelView(User, db.session, name='Admin Users')) # Adding Admin Users view
admin.add_view(CommunityMemberView(CommunityMember, db.session, name='Members'))
...
if __name__ == '__main__':
    with app.app_context():
        ...
        admin_username = os.getenv('ADMIN_USERNAME', '') # Changed default to 'user'
        admin_password = os.getenv('ADMIN_PASSWORD', '') # Changed default password
